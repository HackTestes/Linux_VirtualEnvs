...
# -i input -> input data
# -c:v libaom-av1 -> use AV1 for encoding the video
# -c:a mp3 -> use mp3 to encode the audio
...
ls *.mp3 | sed 's/\..*//' | xargs -i --max-procs=0 bash -c "echo 'Generating {}.mp4...' && ffmpeg -f lavfi -i color=c=black:s=1x1:r=1 -i {}.mp3 -preset slow -y -shortest -c:v libaom-av1 -c:a mp3 -r 1 {}.mp4 && echo 'Finished {}.mp4...'"
